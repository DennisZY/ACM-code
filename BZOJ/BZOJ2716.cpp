...
void cal(int s, int e, int mid, int de, int dx, int dy) {
    for (int i = s; i != e; i += de) {
        if ((b[i].z == 1 && b[i].id > mid) || (b[i].z == 2 && b[i].id <= mid)) continue;
        int y = dy == 1 ? b[i].y : tot - b[i].y;
        int tmp = dx * b[i].x + dy * b[i].y;
        if (b[i].z == 1)insert(y, tmp);
        else ans[b[i].id] = min(ans[b[i].id], abs(tmp - ask(y)));
    }
    for (int i = s; i != e; i += de) {
        if ((b[i].z == 1 && b[i].id > mid) || (b[i].z == 2 && b[i].id <= mid)) continue;
        int y = dy == 1 ? b[i].y : tot - b[i].y;
...
void CDQ(int l, int r) {
    ...
    int mid = (l + r) >> 1;
    if (l <= mid)CDQ(l, mid);
    if (mid < r)CDQ(mid + 1, r);
    int top = l, x = l, y = mid + 1;
    while (x <= mid && y <= r) {
        if (b[x] < b[y])tmp[top++] = b[x++];
        else tmp[top++] = b[y++];
    }
    while (x <= mid)tmp[top++] = b[x++];
    while (y <= r)tmp[top++] = b[y++];
    for (int i = l; i <= r; i++) {
        b[i] = tmp[i];
    }
    cal(l, r + 1, mid, 1, 1, 1);
