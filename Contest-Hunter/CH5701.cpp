...
int main()
{
    ...
    for (int i = 1; i <= n; i++) {
        if (nxt[1][i])
            dp[0][i][1] = nxt[1][i], da[0][i][1] = 0,
            db[0][i][1] = abs(h[nxt[1][i]] - h[i]);
        if (nxt[0][i])
            dp[0][i][0] = nxt[0][i], da[0][i][0] = abs(h[nxt[0][i]] - h[i]),
            db[0][i][0] = 0;
    }
    ...
    for (int i = 1; i <= w; i++) {
        for (int j = 1; j <= n; j++) {
            for (int k = 0; k <= 1; k++) {
                ...
                if (dp[i][j][k]) {
                    da[i][j][k] =
                        da[i - 1][j][k] + da[i - 1][dp[i - 1][j][k]][l];
                    db[i][j][k] =
                        db[i - 1][j][k] + db[i - 1][dp[i - 1][j][k]][l];
                }
            }
        }
    }
