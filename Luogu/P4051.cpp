...
// 调用之前应将*型按要求放入SA中
inline void induced_sort(int *S, int *SA, int *type, int *bucket, int *lbucket, int *sbucket, int n,
                         int SIGMA) {
    for (int i = 0; i <= n; i++)
...
static int *SAIS(int *S, int length, int SIGMA) {
    ...
    int cnt = 0;
    for (int i = 1; i <= n; i++)
        if (type[i] == S_TYPE && type[i - 1] == L_TYPE) position[cnt++] = i;

    // 对LMS子串进行排序
    fill(SA, SA + n + 1, -1);
    for (int i = 0; i < cnt; i++) SA[sbucket[S[position[i]]]--] = position[i];
    ...
    for (int i = 1; i <= n; i++) {
        ...
        if (is_lms_char(type, x)) {
            if (lastx >= 0 && !equal_substring(S, x, lastx, type)) namecnt++;
